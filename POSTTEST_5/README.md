...
7. **Penerapan Konsep Abstraksi**

   Pada tahap ini, konsep abstraksi diterapkan dengan mengubah class induk `Laptop` menjadi **abstract class** sehingga tidak bisa diinstansiasi langsung.
   Class ini berfungsi sebagai cetakan umum laptop, sedangkan detail perilaku ditentukan pada subclass.

   Implementasi abstraksi pada program:
   - `Laptop` diubah menjadi `abstract class`.
   - Method `getKategori()` dan `hitungHargaJual()` dijadikan **abstract method**.
   - Kedua method tersebut wajib diimplementasikan oleh subclass (`LaptopGaming` dan `LaptopBisnis`).
   - Ditambahkan interface `LaptopOperable` (2 method) dan diimplementasikan oleh `Laptop`.
