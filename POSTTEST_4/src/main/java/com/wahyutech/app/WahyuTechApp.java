...
public class WahyuTechApp {
    ...
    void tambahLaptop(ArrayList<Laptop> daftarLaptop, Scanner scanner) {
        ...
        if (jenis == 1) {
            if (diskon > 0) {
                laptopBaru = new LaptopGaming(id, merk, tipe, harga, stok, diskon);
            } else {
                laptopBaru = new LaptopGaming(id, merk, tipe, harga, stok);
            }
        } else if (jenis == 2) {
            if (diskon > 0) {
                laptopBaru = new LaptopBisnis(id, merk, tipe, harga, stok, diskon);
            } else {
    ...
    void tampilkanLaptop(ArrayList<Laptop> daftarLaptop) {
        ...
        System.out.println("--------------------------------------------------------------------------------------------------------------------");
        System.out.printf("| %-5s | %-10s | %-20s | %-10s | %-17s | %-20s | %-4s |\n", "ID", "Merk", "Tipe", "Kategori", "Harga Dasar", "Harga Jual Final", "Stok");
        System.out.println("--------------------------------------------------------------------------------------------------------------------");
    ...
    void updateLaptop(ArrayList<Laptop> daftarLaptop, Scanner scanner) {
        ...
        for (int i = 0; i < daftarLaptop.size(); i++) {
            ...
            if (laptop.getId().equalsIgnoreCase(idCari)) {
                ...
                System.out.print("Diskon baru (%) [0]: ");
                String diskonStr = scanner.nextLine();
                double diskonBaru = diskonStr.isEmpty() ? 0 : Double.parseDouble(diskonStr);
                ...
                if (jenisBaru == 1) {
                    if (diskonBaru > 0) {
                        laptopBaru = new LaptopGaming(id, merkBaru, tipeBaru, hargaBaru, stokBaru, diskonBaru);
                    } else {
                        laptopBaru = new LaptopGaming(id, merkBaru, tipeBaru, hargaBaru, stokBaru);
                    }
                } else {
                    if (diskonBaru > 0) {
                        laptopBaru = new LaptopBisnis(id, merkBaru, tipeBaru, hargaBaru, stokBaru, diskonBaru);
                    } else {
